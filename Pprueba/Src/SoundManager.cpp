...
SoundManager* ms_Singleton = nullptr;
...
void SoundManager::Initialize(void)
{
	...
	system->createChannelGroup("sonidos", &grupo1);
...
int SoundManager::CreateSound(String &fileName, SOUND_TYPE soundType)
{
	...
	{
		result = system->createSound((const char *)newSoundInstance, FMOD_LOOP_NORMAL | FMOD_3D , 0, &sound);
		break;
	}
	...
	{
		result = system->createStream((const char *)newSoundInstance, FMOD_LOOP_NORMAL | FMOD_2D , 0, &sound);
		break;
	}
...
void SoundManager::PlaySound(int soundIndex, SceneNode *soundNode, int *channelIndex)
{
	...
	result = system->playSound(soundInstance->fmodSound,grupo1,false,&channel);
	if (result != FMOD_OK)
	{
